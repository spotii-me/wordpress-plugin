...
jQuery(document).ready(function($){
	...
	spottiCapture = function(orderId,status){
	  $.ajax({
		  type : "post",
		  dataType : "json",
	...
	window.closeIFrameOnCompleteOrder = function({ status }) {
      ...
      var orderCancel = $.cookie('order_cancel');

      $.removeCookie("order_cancel");
	  ...
	  switch (status) {
		case successCheckOutStatus: {
          spottiCapture(orderId,"completed");
		  break;
		}
		case failedCheckOutStatus: {
          spottiCapture(orderId, "canceled");
		  submit_error( '<div class="woocommerce-error">Payment with Spotii failed. Please try again</div>' ); // eslint-disable-line max-len
		  break;
		}
      }
      $.fancybox.close();
